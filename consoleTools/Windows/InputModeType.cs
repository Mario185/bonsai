...
namespace consoleTools.Windows
{
  ...
  internal enum InputModeType
  {
    ...
    ///   ReadConsole buffer. If the ENABLE_LINE_INPUT mode is also enabled, backspace, carriage return, and line feed
    ///   characters are handled by the system.
    /// </summary>
    ENABLE_PROCESSED_INPUT = 1,

    /// <summary>
    ///   The ReadFile or ReadConsole function returns only when a carriage return character is read. If this mode is disabled,
    ///   the functions return when one or more characters are available.
    /// </summary>
    ENABLE_LINE_INPUT = 2,

    /// <summary>
    ///   Characters read by the ReadFile or ReadConsole function are written to the active screen buffer as they are typed
    ///   into the console. This mode can be used only if the ENABLE_LINE_INPUT mode is also enabled.
    /// </summary>
    ENABLE_ECHO_INPUT = 4,
    ...
    ///   generated by mouse movement and button presses are placed in the input buffer. These events are discarded by ReadFile
    ///   or ReadConsole, even when this mode is enabled. The ReadConsoleInput function can be used to read MOUSE_EVENT input
    ...
    ///   that location will not be overwritten. When disabled, all following text will be overwritten.
    /// </summary>
    ENABLE_INSERT_MODE = 32,

    /// <summary>
    ///   This flag enables the user to use the mouse to select and edit text. To enable this mode, use ENABLE_QUICK_EDIT_MODE
    ///   | ENABLE_EXTENDED_FLAGS. To disable this mode, use ENABLE_EXTENDED_FLAGS without this flag.
    /// </summary>
    ...
    ///   into Console Virtual Terminal Sequences that can be retrieved by a supporting application through ReadFile or
    ///   ReadConsole functions.
    ///   The typical usage of this flag is intended in conjunction with ENABLE_VIRTUAL_TERMINAL_PROCESSING on the output
